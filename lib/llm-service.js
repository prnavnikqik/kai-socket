 ...
 * Uses Cosine Similarity to find chunks by "meaning" rather than keywords.
*/
export async function searchChunksSemantic(query, allChunks, limit = 10) {
    try {
        const { generateEmbedding, cosineSimilarity } = await import('./vector-service.js');
        const queryVec = await generateEmbedding(query);

        const scored = allChunks
            .filter(c => c.embedding) // Only check chunks with embeddings
            .map(chunk => ({
                ...chunk,
                relevanceScore: cosineSimilarity(queryVec, chunk.embedding)
            }));
        ...
        return searchChunksKeywords(query, allChunks, limit);
    }
}
